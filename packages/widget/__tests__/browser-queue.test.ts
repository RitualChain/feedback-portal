...
function installQueueStub(): void {
  ...
  queueFn.q = q
  window.RitualChain = queueFn
}

describe('browser-queue', () => {
  beforeEach(() => {
    vi.resetModules()
    delete (window as Window).RitualChain
  ...
  it('replays queued commands through the SDK after the script loads', async () => {
    ...
    installQueueStub()
    window.RitualChain!('init', { instanceUrl: 'https://feedback.acme.com' })
    window.RitualChain!('identify', { id: 'u1', email: 'a@b.c' })
  ...
  it('replaces window.RitualChain with a live dispatcher after loading', async () => {
    vi.doMock('../src/core/sdk', () => ({
      createSDK: () => ({
        dispatch: () => 'DISPATCHED',
        isOpen: () => false,
    ...
    expect(typeof window.RitualChain).toBe('function')
    expect((window.RitualChain as (...args: unknown[]) => unknown)('open')).toBe('DISPATCHED')
  })
  ...
  it('folds the baked instance URL into a queued init that omits it', async () => {
    ...
    window.RitualChain!('init', { launcher: false, placement: 'left' })
  ...
  it('folds the baked instance URL into a bare queued `RitualChain("init")` call', async () => {
    const dispatched = mockSDK()
    window.__QUACKBACK_URL__ = 'https://feedback.acme.com'
    installQueueStub()
    // The documented admin snippet — no second argument at all.
    window.RitualChain!('init')
  ...
  it('end-to-end: fallback first, then a host init({launcher:false, placement:"left"}) destroys + rebuilds', async () => {
    ...
    const styles = document.getElementById('ritualchain-widget-styles')?.textContent ?? ''
  ...
  it('end-to-end: a post-load init({launcher:false, placement:"left"}) reconfigures the DOM', async () => {
    ...
    expect(document.querySelector('button[aria-label="Open feedback widget"]')).toBeNull()
    expect(document.querySelector('.ritualchain-panel')).not.toBeNull()
    const styles = document.getElementById('ritualchain-widget-styles')?.textContent ?? ''
    expect(styles).toContain('left:24px')
    expect(styles).not.toContain('right:24px')
  })
  ...
  it('does not auto-init if the queue already contains an init call', async () => {
    ...
    window.__QUACKBACK_URL__ = 'https://feedback.acme.com'
    installQueueStub()
    window.RitualChain!('init', { instanceUrl: 'https://override.example' })
