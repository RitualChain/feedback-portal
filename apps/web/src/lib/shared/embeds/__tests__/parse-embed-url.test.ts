...
// (per @ritualchain/ids ID_PREFIXES), not `clog_`, and isValidTypeId rejects
...
describe('parseEmbedUrl', () => {
  it('parses a post url', () => {
    expect(parseEmbedUrl(`https://acme.ritual.net/b/features/posts/${POST_ID}`)).toEqual({
      kind: 'post',
      id: POST_ID,
    })
  })
  it('parses a changelog url', () => {
    expect(parseEmbedUrl(`https://acme.ritual.net/changelog/${CHANGELOG_ID}`)).toEqual({
      kind: 'changelog',
      id: CHANGELOG_ID,
    })
  })
  it('rejects a non-typeid id', () => {
    expect(parseEmbedUrl('https://acme.ritual.net/b/x/posts/not-an-id')).toBeNull()
  })
  it('rejects a typeid of the wrong kind', () => {
    // A post id sitting on the changelog path must not parse as a changelog.
    expect(parseEmbedUrl(`https://acme.ritual.net/changelog/${POST_ID}`)).toBeNull()
  })
  it('ignores unrelated urls', () => {
    expect(parseEmbedUrl('https://youtube.com/watch?v=abc')).toBeNull()
    expect(parseEmbedUrl('https://acme.ritual.net/?board=features')).toBeNull()
...
describe('paste regexes', () => {
  ...
  it('POST_URL_PASTE_RE captures the post id from a full url', () => {
    const m = new RegExp(POST_URL_PASTE_RE).exec(
      `https://acme.ritual.net/b/features/posts/${POST_ID}`
    )
  ...
  it('CHANGELOG_URL_PASTE_RE captures the changelog id from a full url', () => {
    const m = new RegExp(CHANGELOG_URL_PASTE_RE).exec(
      `https://acme.ritual.net/changelog/${CHANGELOG_ID}`
    )
