...
import { RitualChainEmbedCard } from '@/components/shared/ritualchain-embed-card'
import { cn } from '@/lib/shared/utils'

export interface RitualChainEmbedOptions {
...
/**
 * In-editor render for a RitualChain link embed. Reads the `{ kind, id }` off the
 * node and defers to the shared, viewer-scoped {@link RitualChainEmbedCard}. The
 ...
 */
function RitualChainEmbedNodeView({ node, selected, deleteNode }: ReactNodeViewProps) {
  const kind = node.attrs.kind as 'post' | 'changelog' | 'article' | null
  const id = node.attrs.id as string | null
  return (
    <NodeViewWrapper
      className={cn(
        'ritualchain-embed-nodeview group relative my-2 inline-block max-w-full rounded-md',
        selected && 'ring-2 ring-ring ring-offset-1'
      )}
      contentEditable={false}
    >
      {/* Inert preview while composing — no live voting, no navigation. */}
      {kind && id ? <RitualChainEmbedCard kind={kind} id={id} interactive={false} /> : null}
...
/**
 * A first-party TipTap node for embedding RitualChain posts / changelog entries.
 * An atom block (no editable children) that paste rules create from a pasted
 * URL. Serializes to `<div data-ritualchain-embed data-kind data-id>` so saved
 * content round-trips through any editor that has this node in its schema.
 */
export const RitualChainEmbed = Node.create<RitualChainEmbedOptions>({
  name: 'ritualchainEmbed',
  group: 'block',
  atom: true,
  selectable: true,
  draggable: true,
  ...
  addNodeView() {
    return ReactNodeViewRenderer(RitualChainEmbedNodeView)
  },
